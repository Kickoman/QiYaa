...
Переменная `QIYAA_AUDIO_BACKEND` выбирает звуковую подсистему miniaudio по имени: `pulseaudio`, `alsa`, `wasapi`, `coreaudio`… или `null` — без звука, но время идёт как при воспроизведении. Тесты в CTest всегда идут через `null`, чтобы не зависеть от звуковой карты машины.
